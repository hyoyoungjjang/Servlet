...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//위의 결과의따라 응답페이지(html)만들어서 전송
		//즉, 여기 Java코드 내에 사용자가 보게될 응답 html 구문을 작성
		
		//장점 : java코드 내에 작성하기 때문에 반복문이나 조건문, 유용한 메소드 같은 것들을 활용하여 동적인 응답을 내릴 수 있다.
		//단점 : 불편, 복잡, 혹시라도 차우에 html을 수정한다면 java코드를 수정하는 것이기 때문에 수정된 내용을 다시 반영시키고자
		//      서버를 재실행 해야한다.
		
		// reponse객체를 통해 사용자에게 html(응답화면) 전달
		...
		//3) 위에서가져온 스트림을 통해서 응답 html구문을 한줄식 출력;
